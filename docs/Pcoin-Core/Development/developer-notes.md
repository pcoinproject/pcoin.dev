...
- **Indentation and whitespace rules** as specified in
  [src/.clang-format](https://github.com/pcoinproject/pcoin/blob/master/src/.clang-format). You can use the provided
  [clang-format-diff script](https://github.com/pcoinproject/pcoin/tree/master/contrib/devtools#clang-format-diffpy)
...
PCOIN Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play PCOINs (tPCOIN)" on the test network, if you
are testing multi-machine code that needs to operate across the internet.

If you are testing something that can run on one machine, run with the `-regtest` option.
In regression test mode, blocks can be created on-demand; see [test/functional/](https://github.com/pcoinproject/pcoin/tree/master/test/functional) for tests
that run in `-regtest` mode.

### DEBUG_LOCKORDER

PCOIN Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/pcoinproject/pcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_pcoin
...
- BitcoinMiner : Generates PCOINs (if wallet is enabled).
...
However, in open source software such as PCOIN Core, where everyone uses
...
pay attention to for reviewers of PCOIN Core code.
...
- Use `Span` as function argument when it can operate on any range-like container.
  ...
  - _Rationale_: Compared to `Foo(const vector<int>&)` this avoids the need for a (potentially expensive)
    ...
    However, be aware of the pitfalls documented in [span.h](https://github.com/pcoinproject/pcoin/blob/master/src/span.h).
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - _Rationale_: PCOIN Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
There is a tool in `test/lint/git-subtree-check.sh` ([instructions](https://github.com/pcoinproject/pcoin/tree/master/test/lint#git-subtree-checksh)) to check a subtree directory for consistency with
...
- src/leveldb
  ...
  - :::note
    Follow the instructions in [Upgrading LevelDB](#upgrading-leveldb) when
...
file descriptors it will cause problems with PCOIN's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - _Rationale_: This is impossible to use with `pcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - _Rationale_: `pcoin-cli` and the GUI debug console use this table to determine how to
